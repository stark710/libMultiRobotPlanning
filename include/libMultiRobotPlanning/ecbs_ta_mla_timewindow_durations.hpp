...
#include "ml_a_star_durations.hpp"
...
class ECBSTA {
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    start.id = 0;
    start.isRoot = true;
    m_env.nextTaskAssignment(start.tasks, start.taskGoalStates, start.taskGoalTimes, start.taskGoalDurations); // First assignment 
    ...
    for (size_t i = 0; i < numAgents; ++i) {
      ...
      if (i < solution.size() && solution[i].states.size() > 1) {
        ...
        LowLevelEnvironment llenv(m_env, i, start.constraints[i],start.taskGoalStates, start.taskGoalTimes, start.taskGoalDurations, start.solution);
...
// check focal list/open list consistency
      ...
      for (const auto& c : constraints) {
        ...
        LowLevelEnvironment llenv(m_env, i, newNode.constraints[i],
                                  newNode.taskGoalStates, newNode.taskGoalTimes, newNode.taskGoalDurations, newNode.solution);
        LowLevelSearch_t lowLevel(llenv);
...
// Check if the current node is the root node 
      ...
      if (open.top().cost > nextRootNodeCost) {
        ...
        HighLevelNode n;
        m_env.nextTaskAssignment(n.tasks, n.taskGoalStates, n.taskGoalTimes, n.taskGoalDurations);
        ...
        if (n.tasks.size() > 0) {
          ...
          for (size_t i = 0; i < numAgents; ++i) {
            LowLevelEnvironment llenv(m_env, i, n.constraints[i], n.taskGoalStates, n.taskGoalTimes, n.taskGoalDurations, n.solution);
            LowLevelSearch_t lowLevel(llenv);
...
// boost::heap::compare<compareFocalHeuristic> > focalSet_t;
  ...
  struct HighLevelNode {
    ...
    std::map<std::string, std::vector<std::vector<int>>> taskGoalTimes; 
    std::map<std::string, std::vector<int>> taskGoalDurations; 
    Cost cost;
  ...
  struct LowLevelEnvironment {
    LowLevelEnvironment(
        ...
        std::map<std::string, std::vector<std::vector<int>>> agents_time_windows_map,
        std::map<std::string, std::vector<int>> agents_time_durations_map,
        ...
        : m_env(env), m_goal_label(0)
          ...
          ,
          m_solution(solution) {
      m_env.setLowLevelContext(agentIdx, &constraints, agent_goals_map, agents_time_windows_map, agents_time_durations_map);
    ...
    std::pair<bool,bool> isSolutionDurations(const State& s, int current_goal_label, int env_goal_label) { return m_env.isSolutionDurations(s, current_goal_label, env_goal_label); }
    ...
    void getNeighborsWithDurations(const State& s,
                      std::vector<Neighbor<State, Action, Cost> >& neighbors, bool reachedGoal, bool withinDuration) {
      m_env.getNeighborsWithDurations(s, neighbors, reachedGoal, withinDuration);
    }
