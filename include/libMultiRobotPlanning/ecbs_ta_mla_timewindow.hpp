...
class ECBSTA {
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    start.id = 0;
    start.isRoot = true;
    m_env.nextTaskAssignment(start.tasks, start.taskGoalStates, start.taskGoalTimes); // First assignment 
    ...
    for (size_t i = 0; i < numAgents; ++i) {
      ...
      if (i < solution.size() && solution[i].states.size() > 1) {
        ...
        LowLevelEnvironment llenv(m_env, i, start.constraints[i],start.taskGoalStates, start.taskGoalTimes, start.solution);
...
// check focal list/open list consistency
      ...
      for (const auto& c : constraints) {
        ...
        LowLevelEnvironment llenv(m_env, i, newNode.constraints[i],
                                  newNode.taskGoalStates, newNode.taskGoalTimes, newNode.solution);
        LowLevelSearch_t lowLevel(llenv);
...
// Check if the current node is the root node 
      ...
      if (open.top().cost > nextRootNodeCost) {
        ...
        HighLevelNode n;
        m_env.nextTaskAssignment(n.tasks, n.taskGoalStates, n.taskGoalTimes);
        ...
        if (n.tasks.size() > 0) {
          ...
          for (size_t i = 0; i < numAgents; ++i) {
            LowLevelEnvironment llenv(m_env, i, n.constraints[i], n.taskGoalStates, n.taskGoalTimes, n.solution);
            LowLevelSearch_t lowLevel(llenv);
  ...
  struct LowLevelEnvironment {
    LowLevelEnvironment(
        ...
        : m_env(env), m_goal_label(0)
          ...
          ,
          m_solution(solution) {
      m_env.setLowLevelContext(agentIdx, &constraints, agent_goals_map, agents_time_windows_map);
