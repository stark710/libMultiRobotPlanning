...
#include <fstream>
#include <iostream>
#include <libMultiRobotPlanning/ecbs_ta_mla_timewindow_durations.hpp>
...
struct State {
  ...
  State(int time, int x, int y)
      : time(time), x(x), y(y), label(-1), duration(0) {}
  State(int time, int x, int y, int label)
      : time(time), x(x), y(y), label(label), duration(0) {}
  State(int time, int x, int y, int label, int duration)
      : time(time), x(x), y(y), label(label), duration(duration) {}
...
class Environment {
 public:
  Environment(size_t dimx, size_t dimy,
              ...
              std::map<std::string, std::vector<int>> task_duration_definition)
      : m_dimx(dimx),
        m_dimy(dimy),
        ...
        m_task_duration_definition(task_duration_definition) {
  ...
  void goalTimesMapping() {
    ...
    for (int i = 0; i < int(m_goals.size()); i++) {
      ...
      m_goal_durations_map[m_goals[i]] = m_goal_durations[i];
    }
  }
  ...
  void setLowLevelContext(
      ...
      std::map<std::string, std::vector<int>> agent_goals_durations_map) {
    assert(constraints);
    m_agentIdx = agentIdx;
    m_goals = agent_goals_map[std::to_string(m_agentIdx)];
    m_goal_times = agent_goals_time_map[std::to_string(m_agentIdx)];
    m_goal_durations = agent_goals_durations_map[std::to_string(m_agentIdx)];
    goalTimesMapping();
    m_goal_label = 0;
  ...
  //     const State& s1a, const State& s1b, int /*gScoreS1a*/, int
  //     /*gScoreS1b*/, const std::vector<PlanResult<State, Action, int> >&
  //     solution) {
  //   int numConflicts = 0;
  //   for (size_t i = 0; i < solution.size(); ++i) {
  //     if (i != m_agentIdx && solution[i].states.size() > 0) {
  ...
  int m_goal_label = 0;
  // int admissibleHeuristic(const State& s) {
  //   return std::abs(s.x - m_goals[m_goal_label].x) + std::abs(s.y -
  //   m_goals[m_goal_label].y);
  // }
  // heuristic = dist(state, current goal) + pairwise sum of dist(current goal,
  // next goal) from current goal to last goal

  int admissibleHeuristic(const State& s) {
    int h = 0;
    int current_goal = m_goal_label;
    int next_goal = current_goal + 1;
    h += std::abs(s.x - m_goals[current_goal].x) +
         std::abs(s.y - m_goals[current_goal].y);
    while (next_goal < m_num_of_goals) {
      h += std::abs(m_goals[current_goal].x - m_goals[next_goal].x) +
           std::abs(m_goals[current_goal].y - m_goals[next_goal].y);
  ...
  std::pair<bool, bool> isSolutionDurations(const State& s,
                                            int current_goal_label,
                                            int env_goal_label) {
    bool atGoal = false;
  ...
  bool isGoalWithinDuration(const State& goalState, int env_goal_label) {
    State current_goal = m_goals[env_goal_label];
    return goalState.duration < m_goal_durations_map[current_goal];
  }

  void getNeighborsWithDurations(
      const State& s, std::vector<Neighbor<State, Action, int>>& neighbors,
      bool reachedGoal, bool withinDuration) {
    neighbors.clear();
    int new_label = s.label;
    int cost = 1;
    ...
    if (reachedGoal && withinDuration) {
      ...
      if (stateValid(up)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(up, Action::Up, cost));
      }
      State down(s.time + 1, s.x, s.y - 1, new_label);
      if (stateValid(down)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(down, Action::Down, cost));
      }
      State left(s.time + 1, s.x - 1, s.y, new_label);
      if (stateValid(left)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(left, Action::Left, cost));
      }
      State right(s.time + 1, s.x + 1, s.y, new_label);
      if (stateValid(right)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(right, Action::Right, cost));
      }
    }
  ...
  //     if (stateValid(n) && transitionValid(s, n)) {
  //       neighbors.emplace_back(Neighbor<State, Action, int>(n, Action::Up,
  //       1));
  //     }
  //   }
  //   {
  ...
  std::map<std::string, std::vector<State>> taskToGoals(
      std::map<std::string, std::string> agent_task_map) {
    std::map<std::string, std::vector<State>> agent_goals_map;
  ...
  std::map<std::string, std::vector<std::vector<int>>> taskToGoalTimes(
      std::map<std::string, std::string> agent_task_map) {
    std::map<std::string, std::vector<std::vector<int>>> agent_goal_times_map;

    for (auto it = agent_task_map.begin(); it != agent_task_map.end(); ++it) {
      std::string task_id = it->second;
      std::vector<std::vector<int>> task_goal_times =
          m_task_time_definition[task_id];
  ...
  std::map<std::string, std::vector<int>> taskToGoalDurations(
      std::map<std::string, std::string> agent_task_map) {
    std::map<std::string, std::vector<int>> agent_goal_durations_map;

    for (auto it = agent_task_map.begin(); it != agent_task_map.end(); ++it) {
      std::string task_id = it->second;
      std::vector<int> task_goal_durations =
          m_task_duration_definition[task_id];

      agent_goal_durations_map.insert(
          std::make_pair(it->first, task_goal_durations));
    }

    return agent_goal_durations_map;
  }
  ...
  void nextTaskAssignment(
    ...
    agent_goal_durations_map = taskToGoalDurations(tasks);
 ...
 private:
  ...
  std::map<std::string, std::vector<int>> m_task_duration_definition;
  ...
  std::vector<int> m_goal_durations;
  std::unordered_map<State, std::vector<int>> m_goal_times_map;
  std::unordered_map<State, int> m_goal_durations_map;
};
...
int main(int argc, char* argv[]) {
  ...
  for (const auto& node : config["agents"]) {
    ...
    for (int i = 0; i < int(node["potentialGoals"].size()); i++) {
      ...
      sum_cost = m_heuristic.getValue(
                     Location(startStates[agent_id].x, startStates[agent_id].y),
                     Location(all_goals[0][0], all_goals[0][1])) +
                 all_durations[0];
      ...
      for (int j = 1; j < int(all_goals.size()); j++) {
        ...
        curr_cost = m_heuristic.getValue(Location(prev_goal_x, prev_goal_y),
                                         Location(curr_goal_x, curr_goal_y));
        curr_time_cost = all_durations[j];
        sum_cost += curr_cost + curr_time_cost;
      }

      // If the sum of costs is less than the time window, then the agent can
      // reach all goals in time
      int last_goal_deadline = all_time_windows[num_goals - 1][1];
      if (sum_cost < last_goal_deadline) {
        assignment.setCost(std::to_string(agent_id), std::to_string(task_id),
                           sum_cost);
      }

      // If the sum of costs is greater than the time window, then the agent
      // cannot reach all goals in time
      else {
        assignment.setCost(std::to_string(agent_id), std::to_string(task_id),
                           1000000);
      }
